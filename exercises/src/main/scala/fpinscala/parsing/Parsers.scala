...
trait Parsers[Parser[+_]] { self => // so inner classes may call methods of trait
  ...
  /* chosses between 2 parsers, first p1 and then p2 if it s the case */
  def or[A](p1: Parser[A], p2: => Parser[A]): Parser[A]

  def char(c: Char): Parser[Char] = string(c.toString) map (_.charAt(0))

  /* always succ with val a */
  def succeed[A](a: A): Parser[A]

  /* applies f to the result of p if succ */
  def map[A,B](a: Parser[A])(f: A => B): Parser[B] =
    a flatMap(aVal => succeed(f(aVal)))

  def many[A](p: Parser[A]): Parser[List[A]] =
    map2(p, many(p))(_ :: _) or succeed(List())

  /* Returns the portion of string inspected by p if succ */
  def slice[A](p: Parser[A]): Parser[String]

  def many1[A](p: Parser[A]): Parser[List[A]] =
    self.map2(p, many(p))(_ :: _)

  /* sequences two parsers and then returns the pair of their res if succ */
  ...
  val digitFollowedByA = flatMap("[0-9]".r)(s => listOfN(s.toInt, char('a')))
  ...
  object Laws {
    ...
    def unitLaw[A](a: A)(in: Gen[String]): Prop =
      forAll(in)(run(succeed(a))(_) == Right(a))

    def productLaw[A, B](p1: Parser[A], p2: Parser[B])(in: Gen[String]): Prop =
      forAll(in)(s => (run(p1)(s), run(p2)(s)) == run(p1 ** p2)(s))
  }
}
